...
export async function logAudit(req, action, category, details) {
  try {
    ...
    await pool.query(
      'DECLARE @NextLogID INT = ISNULL((SELECT MAX(LogID) FROM AuditLogs WITH (UPDLOCK, HOLDLOCK)), 0) + 1; INSERT INTO AuditLogs (LogID, AdminID, AdminUsername, Action, Category, Details, IPAddress) VALUES (@NextLogID, ?, ?, ?, ?, ?, ?)',
